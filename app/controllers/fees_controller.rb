...
#com
  ...
  def section_students
    ...
    studentsList1=MgStudent.where(:mg_batch_id=>@ids,:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
    @studentsList=Array.new
    studentsList1.each do|data|
    student_data2=Array.new
      first_name=data.first_name
      last_name=data.last_name
      student_data2.push("#{data.admission_number} - #{first_name} #{last_name}",data.id)
      @studentsList.push(student_data2)
  ...
  def fee_discounts
    ...
    if request.xhr?
      ...
      if(ajaxParamAdmissionNumber=="admissionNumberParam")
        ...
        @validAdmissionNumber=MgStudent.where(:admission_number => params[:admissionNumber],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
  ...
  def fee_schedule
    ...
    if request.post?
        ...
        @selected_batch_id_array .each do|batchId|
            ...
            if params[:dataaa]=="canteen_management" 
               @mg_fee_collection.update(:collection_type=>"canteen_management")
            end
        ...
        puts "999999999999999999999999999999999999999999999999999999999999999999999999999999999999"
        puts params[:dataaa]
        puts"================================"
        puts params
        puts "999999999999999999999999999999999999999999999999999999999999999999999999999999999999"
        ...
        elsif params[:dataaa]=="canteen_management"
          redirect_to :action=>'canteen_fee_schedule',:controller=>"canteen_managements"
        else
  ...
  def fee_defaulter
    ...
    if request.xhr?
      ...
      if(taskRequired=="getBatchList")
        ...
        @student_list = MgStudent.select(:id, :first_name, :middle_name, :last_name, :mg_batch_id,:admission_number).where(:mg_batch_id => params[:batchId],:is_deleted=>0,:mg_school_id=>school_id,:id=>defaulter_student_ids)
        ...
        @student_list.each do |currStudent|
         ...
         #puts @financeFee.inspect
              ...
              student['admission_number'] = currStudent.admission_number
      ...
      MgFinanceTransaction.transaction do
        ...
        unless params[:amount].nil?
          ...
          collection_particular_ids.each_with_index do |collection_particular,i|
            ...
            particular_type_id=MgParticularType.where(:particular_name=>collection.mg_fee_particular_name,:is_deleted=>0,:mg_school_id=>session[:current_user_school_id])
            to_account=MgFeeParticular.where(:mg_particular_type_id=>particular_type_id[0].id,:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:mg_account_id)
            is_to_central=MgFeeParticular.where(:mg_particular_type_id=>particular_type_id[0].id,:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:is_to_central)
            from_account_id=""
        ...
        if params[:fee_collection_ids].present?
          params[:fee_collection_ids].each do |id|
            total = MgFeeCollectionParticular.where(:mg_fee_collection_id=>id,:mg_student_id=>student_id,:mg_school_id=>school_id,:is_deleted=>0).sum(:mg_fee_particular_amount)      
            paid  = MgFinanceTransactionDetail.where('mg_collection_id =? AND mg_student_id=? AND mg_school_id=? AND is_deleted=? AND mg_fee_particular_id  IS NOT NULL',id,student_id,school_id,0).sum(:paid_amount)
            if(total==paid)
             finance_fee = MgFinanceFee.find_by(:mg_fee_collection_id=>id,:student_id=>student_id,:mg_school_id=>school_id,:is_deleted=>0)
...
#jayanth modification is required
      paid_finance_fee = MgFinanceFee.where(:student_id =>@studentObj.id,:mg_fee_collection_id=>params[:Collectionid],:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:mg_fee_collection_id)
...
# jayanth end modification is required
      ...
      paid_fines =  MgFinanceTransactionDetail.where(:mg_student_id=>@studentObj.id,:mg_collection_id=>nil,:is_deleted=>0,:mg_school_id=>session[:current_user_school_id]).pluck(:mg_fee_fine_particular_id)      
...
render :layout => false
  ...
  def edit
    ...
    @mg_fee_category_batch_list=MgFeeCategoryBatches.where(:mg_fee_id=>params[:id],:is_deleted => 0, :mg_school_id=> session[:current_user_school_id]).pluck(:mg_batch_id,:id)
    @dueFine=MgParticularType.where(:mg_fee_category_id=>params[:id],:is_deleted => 0, :mg_school_id=> session[:current_user_school_id])
...
#logger.infosd
  ...
  def delete_fee_category
    ...
    @fees.is_deleted =1
    @fees.save
    @particularType=MgParticularType.where(:mg_fee_category_id=>params[:id],:is_deleted => 0, :mg_school_id=> session[:current_user_school_id])
...
# end
  ...
  def generate_fee_pdf
    ...
    fee_transaction_detail = MgFinanceTransactionDetail.where(:mg_transaction_id=>fee_transaction.id,:is_deleted => 0, :mg_school_id=> session[:current_user_school_id])
    fine_detail = MgFinanceTransactionDetail.where(:mg_transaction_id=>fee_transaction.id,:detail_type=> 'fine',:is_deleted => 0, :mg_school_id=> session[:current_user_school_id])
    
    collection_ids = fee_transaction_detail.where("is_deleted=0 and mg_school_id=#{session[:current_user_school_id]} and mg_collection_id is not null").pluck("DISTINCT mg_collection_id")
...
#       end
        ...
        collection_ids.each do |c_id|
          ...
          particular_list = fee_transaction_detail.where(:detail_type=> 'particular' ,:mg_collection_id =>c_id,:is_deleted=>0,:mg_school_id=>school.id)
          discount_list   = fee_transaction_detail.where(:detail_type=> 'discount' ,:mg_collection_id =>c_id,:is_deleted=>0,:mg_school_id=>school.id)
          late_fine_list  = fee_transaction_detail.where(:detail_type=> 'late' ,:mg_collection_id =>c_id,:is_deleted=>0,:mg_school_id=>school.id)
          collection_id =c_id
          puts "c_id"
          puts c_id
          move_down 5
